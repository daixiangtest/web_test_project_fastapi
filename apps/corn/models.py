...
class CornJob(models.Model):
    ...
    task = fields.ForeignKeyField("models.TestTasks", related_name="corn", description="执行的测试任务")
    ...
    interval = fields.IntField(default=60, description="执行间隔时间", null=True, blank=True)
    date = fields.DatetimeField(default='2030-01-01 00:00:00', description="指定执行的事件", null=True, blank=True)
    crontab = fields.JSONField(default={'minute': '30', 'hour': '*', 'day': '*', 'month': '*', 'day_of_week': '*'},
                               description="周期性任务规则", null=True, blank=True)
